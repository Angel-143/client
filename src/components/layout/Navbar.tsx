...
export function Navbar() {
  ...
  const indicatorStyle = (() => {
    ...
    return { opacity: 1, x: navRects[i].left - parent.left, width: navRects[i].width };
  })();

  return (
    <header className={cn('sticky top-0 z-50 w-full transition-all duration-300', scrolled ? 'border-b border-slate-200/70 bg-white/80 backdrop-blur-xl dark:border-slate-800/70 dark:bg-slate-950/80' : 'border-b border-transparent bg-transparent')}>
          ...
          {!isAuth ? (
            ...
            <div className="relative hidden sm:block">
              <button onClick={() => setMenuOpen((v) => !v)} className="flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-sm font-medium transition-all hover:shadow-soft active:scale-95 dark:border-slate-700 dark:bg-slate-900">
                <span className="flex h-7 w-7 items-center justify-center rounded-full bg-gradient-to-br from-brand-500 to-violet-500 text-xs font-bold text-white">{profile?.full_name?.[0]?.toUpperCase() ?? 'U'}</span>
              ...
              <AnimatePresence>
                {menuOpen && (
                  <motion.div initial={{ opacity: 0, y: 8, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 8, scale: 0.96 }} transition={{ duration: 0.15 }}
                    ...
                    <button onClick={handleSignOut} className="flex w-full items-center gap-2 px-4 py-2 text-sm text-error-700 transition-colors hover:bg-error-50 dark:text-red-300 dark:hover:bg-red-500/10"><LogOut size={16} /> Sign out</button>
                  </motion.div>
                )}
              </AnimatePresence>
            </div>
            ...
            <div className="container-page space-y-1 py-4">
              ...
              {!isAuth ? (
                ...
                <div className="space-y-1">
                  ...
                  {isAdmin && <Link to="/admin" className="flex items-center gap-2 rounded-lg px-3 py-2.5 text-sm font-medium text-slate-700 transition-colors hover:bg-slate-50 dark:text-slate-200 dark:hover:bg-slate-800"><Shield size={16} /> Admin Dashboard</Link>}
                  <button onClick={handleSignOut} className="flex w-full items-center gap-2 rounded-lg px-3 py-2.5 text-left text-sm font-medium text-error-700 transition-colors hover:bg-error-50 dark:text-red-300 dark:hover:bg-red-500/10"><LogOut size={16} /> Sign out</button>
                </div>
              )}
            </div>
          </motion.div>

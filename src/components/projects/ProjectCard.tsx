...
export default function ProjectCard({ project, index = 0 }: { project: Project; index?: number }) {
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: index * 0.05 }} className="group bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 overflow-hidden hover:shadow-xl hover:shadow-gray-200/50 dark:hover:shadow-black/30 transition-all duration-300 hover:-translate-y-1">
      ...
      <div className="p-5">
        ...
        <div className="flex flex-wrap gap-1.5 mb-4">
          {project.tech_stack.slice(0, 3).map(tech => <span key={tech} className="text-xs px-2 py-0.5 rounded-md bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 font-medium">{tech}</span>)}
          {project.tech_stack.length > 3 && <span className="text-xs px-2 py-0.5 text-gray-400">+{project.tech_stack.length - 3}</span>}
        </div>
        <div className="flex items-center justify-between pt-4 border-t border-gray-100 dark:border-gray-800">
          <div className="flex items-center gap-3">
            <span className="text-lg font-bold text-gray-900 dark:text-white">{formatPrice(project.price)}</span>
            <span className="text-xs text-gray-400 flex items-center gap-1"><Download size={12} /> {formatNumber(project.sales_count)}</span>
          </div>
          <Link to={`/projects/${project.slug}`} className={cn('inline-flex items-center gap-1 text-sm font-medium text-brand-600 dark:text-brand-400 group-hover:gap-2 transition-all')}>View <ArrowRight size={14} /></Link>
        </div>
      </div>
    </motion.div>
  )

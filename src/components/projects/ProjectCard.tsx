      ...
      <div className="flex flex-1 flex-col space-y-3 p-5">
        ...
        <div className="flex flex-wrap gap-1.5">
          {project.tech_stack.slice(0, 3).map((tech) => (
            <span key={tech} className="inline-flex items-center gap-1 rounded-md bg-slate-100 px-2 py-0.5 text-[11px] font-medium text-slate-600 dark:bg-slate-800 dark:text-slate-300">
              <Code2 size={10} /> {tech}
            </span>
          ))}
        ...
        <div className="mt-auto flex items-center justify-between border-t border-slate-100 pt-4 dark:border-slate-800">
          ...
          <div className="flex gap-1.5">
            <button aria-label="Add to favorites" className={cn('flex h-9 w-9 items-center justify-center rounded-lg border border-slate-200 text-slate-500 transition-all hover:scale-105 hover:border-rose-300 hover:text-rose-500 active:scale-95 dark:border-slate-700 dark:text-slate-400')}>
              <Heart size={16} />
            </button>
            <Link to={`/projects/${project.slug}`} className="flex h-9 items-center gap-1.5 rounded-lg bg-brand-600 px-3 text-sm font-semibold text-white shadow-glow transition-all hover:bg-brand-700 active:scale-95">

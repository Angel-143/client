...
export default function AdminDashboard() {
  const { data: stats, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return { projects: projects.data ?? [], orders: orders.data ?? [], messages: messages.data ?? [], profiles: profiles.data ?? [] };
    },
  });
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        {isLoading ? Array.from({ length: 4 }).map((_, i) => <Skeleton key={i} className="h-28 rounded-2xl" />) : cards.map((c, i) => (
          <motion.div key={c.label} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}>
            <div className="card p-5">
              <div className="flex items-center justify-between">
                <div><p className="text-sm text-slate-500 dark:text-slate-400">{c.label}</p><p className="mt-1 font-display text-2xl font-bold">{c.value}</p>{c.sub && <p className="text-xs text-brand-600 dark:text-brand-400">{c.sub}</p>}</div>
                <div className={`flex h-11 w-11 items-center justify-center rounded-xl ${c.color}`}><c.icon size={20} /></div>
              </div>
              {c.link && <Link to={c.link} className="mt-3 inline-flex items-center gap-1 text-xs font-medium text-brand-600 hover:text-brand-700 dark:text-brand-400">View <ArrowUpRight size={12} /></Link>}
            </div>
          </motion.div>
        ))}
      </div>
            ...
            {(stats?.projects ?? []).sort((a, b) => b.sales_count - a.sales_count).slice(0, 5).map((p) => (
              <div key={p.id} className="flex items-center justify-between p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div><p className="text-sm font-semibold text-slate-900 dark:text-white">{p.title}</p><p className="flex items-center gap-1 text-xs text-slate-500"><TrendingUp size={12} /> {formatNumber(p.sales_count)} sales</p></div>
                </div>
          ...
          <div className="card divide-y divide-slate-100 dark:divide-slate-800">
            {(stats?.messages ?? []).slice(0, 5).map((m) => (
              <div key={m.id} className="p-4">
                <div className="flex items-center justify-between"><p className="text-sm font-semibold text-slate-900 dark:text-white">{m.name}</p><Badge variant={m.status === 'new' ? 'brand' : 'neutral'}>{m.status}</Badge></div>

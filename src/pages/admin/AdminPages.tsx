        ...
        <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden">
          <div className="divide-y divide-gray-100 dark:divide-gray-800 max-h-[600px] overflow-y-auto">
            {messages?.map((msg) => (
              <button
                key={msg.id}
                onClick={() => { setSelected(msg); if (msg.status === 'new') markRead(msg) }}
                className={`w-full text-left p-4 hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors ${selected?.id === msg.id ? 'bg-brand-50 dark:bg-brand-900/20' : ''}`}
              >
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    <p className="font-medium text-gray-900 dark:text-white text-sm truncate">{msg.name}</p>
                    <p className="text-xs text-gray-500 truncate">{msg.email}</p>
                    <p className="text-xs text-gray-400 mt-1 truncate">{msg.subject || msg.message}</p>
                  </div>
                  <div className="flex-shrink-0">
                    {msg.status === 'new' ? <Badge color="brand">New</Badge> : <Badge color="gray">Read</Badge>}
                  </div>
                </div>
              </button>
            ))}
            {(!messages || messages.length === 0) && (
              <p className="p-8 text-center text-gray-500 dark:text-gray-400 text-sm">No messages yet.</p>
            )}
          </div>
        </div>
          ...
          {selected ? (
            <div>
              <div className="flex items-start justify-between mb-4">
                ...
                <button onClick={() => handleDelete(selected.id)} className="p-2 text-gray-400 hover:text-red-600">
                  <Trash2 size={16} />
                </button>
              </div>
            ...
            <div className="flex flex-col items-center justify-center h-full py-16 text-center">
              ...
              <p className="text-gray-500 dark:text-gray-400 text-sm">Select a message to view details</p>
            </div>
          )}
        </div>
      </div>

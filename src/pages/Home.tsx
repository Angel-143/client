...
import {
  ArrowRight, ArrowUpRight, ShieldCheck, Zap, Code2, Headphones, RefreshCw,
          ...
          <div className="grid gap-12 lg:grid-cols-12 lg:gap-8">
            ...
            <div className="lg:col-span-7 lg:pt-6">
              <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
                className="group inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white/70 px-3 py-1.5 text-xs font-medium text-slate-700 backdrop-blur dark:border-slate-800 dark:bg-slate-900/70 dark:text-slate-300">
              ...
              <motion.h1 initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.05 }}
                className="mt-6 max-w-3xl font-display text-4xl font-extrabold leading-[1.05] tracking-tight text-balance sm:text-5xl lg:text-6xl">
              ...
              <motion.p initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 }}
                className="mt-6 max-w-xl text-lg text-slate-600 dark:text-slate-300 text-balance">
              ...
              <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.15 }}
                ...
                <Link to="/about" className="btn-secondary group px-6 py-3 text-base">
                  Learn more
                  <ArrowUpRight size={16} className="text-slate-400 transition-all group-hover:translate-x-0.5 group-hover:text-brand-600" />
                </Link>
              </motion.div>
                ...
                <div className="relative rounded-2xl border border-slate-200/80 bg-white/90 p-5 shadow-card backdrop-blur-xl dark:border-slate-800/80 dark:bg-slate-900/80">
                  ...
                  <div className="space-y-3">
                    {[
                      ...
                      <motion.div key={row.name} initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4 + i * 0.12 }}
                        className="group flex items-center gap-3 rounded-xl border border-slate-100 bg-slate-50/80 p-3 transition-colors hover:border-brand-200 hover:bg-white dark:border-slate-800 dark:bg-slate-800/40 dark:hover:bg-slate-800">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="truncate text-xs text-slate-500 dark:text-slate-400">{row.tag}</p>
                        </div>
                  ...
                  <div className="mt-4 flex items-center justify-between rounded-xl bg-gradient-to-r from-brand-600 to-violet-600 px-4 py-2.5 text-white">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-xs text-white/80">/ 5 · {formatNumber(totalSales)}+ sales</span>
                    </div>
                    <TrendingUp size={16} />
                  </div>
                </div>

                {/* Floating chip: Instant download (top-right) */}
                <motion.div animate={{ y: [0, -8, 0] }} transition={{ duration: 5, repeat: Infinity, ease: 'easeInOut' }}
                  className="absolute -right-4 -top-4 hidden rounded-xl border border-slate-200 bg-white p-3 shadow-card dark:border-slate-800 dark:bg-slate-900 sm:block">
                  <div className="flex items-center gap-2">
                    <Zap size={14} className="text-accent-500" />
                    <span className="text-xs font-semibold text-slate-700 dark:text-slate-200">Instant download</span>
                  </div>
                </motion.div>

                {/* Floating chip: Lifetime updates (bottom-left) — replaces "Code Reviewed" */}
                <motion.div animate={{ y: [0, 8, 0] }} transition={{ duration: 6, repeat: Infinity, ease: 'easeInOut', delay: 1 }}
                  className="absolute -bottom-4 -left-4 hidden rounded-xl border border-slate-200 bg-white p-3 shadow-card dark:border-slate-800 dark:bg-slate-900 sm:block">
                  <div className="flex items-center gap-2">
                    <RefreshCw size={14} className="text-warning-500" />
                    <span className="text-xs font-semibold text-slate-700 dark:text-slate-200">Lifetime updates</span>
                  </div>
                </motion.div>
              </div>
            </motion.div>
      ...
      <section className="section">
        <div className="container-page">
          <div className="mb-10 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-end">
            ...
            <Link to="/projects" className="group inline-flex items-center gap-1.5 text-sm font-semibold text-brand-600 hover:text-brand-700 dark:text-brand-400">
              View all <ArrowRight size={16} className="transition-transform group-hover:translate-x-0.5" />
            </Link>
          </div>
              ...
              {categories.map((c, i) => (
                <motion.div key={c.id} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.05 }}>
                  <Link to={`/projects?category=${c.id}`} className="card-hover group flex flex-col items-center gap-3 p-5 text-center">
                    <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-brand-50 text-brand-600 transition-colors group-hover:bg-brand-600 group-hover:text-white dark:bg-brand-500/10 dark:text-brand-400">
                      <Code2 size={22} />
                    </div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-40px' }} transition={{ duration: 0.5 }}
            ...
            <div className="relative mx-auto max-w-2xl">
              ...
              <div className="mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row">
                <Link to="/register" className="group inline-flex items-center gap-2 rounded-xl bg-white px-6 py-3 text-base font-semibold text-brand-700 shadow-lg transition-transform hover:scale-105">
                  Create free account <ArrowRight size={18} className="transition-transform group-hover:translate-x-0.5" />
                </Link>

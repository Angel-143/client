...
export function useProjects(filters?: { category?: string; search?: string; difficulty?: string; sort?: 'newest' | 'price-low' | 'price-high' | 'popular' | 'rating' }) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (filters?.search) q = q.or(`title.ilike.%${filters.search}%,short_description.ilike.%${filters.search}%`);
      switch (filters?.sort) {
...
export function useFeaturedProjects(limit = 6) {
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from('projects').select('*, category:categories(*)').eq('is_featured', true).order('sales_count', { ascending: false }).limit(limit);
      if (error) throw error;
      return (data ?? []) as Project[];
    },
  });
...
export function useProject(slug: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from('projects').select('*, category:categories(*)').eq('slug', slug).maybeSingle();
      if (error) throw error;
